...
Page({
  ...
  add:function(){
    wx.navigateTo({
      url: '/pages/second_level/class/add/add',
    })
  ...
  editMessage: function(e) {
    ...
    // 例如：跳转到编辑页面并传递ID参数
    wx.navigateTo({
      url: `/pages/second_level/class/edit/edit?id=${id}&name=${name}`
    });
  },
  getInputValue(e) {
    this.setData({
  ...
  loadData: function() {
    ...
    var that = this;
    wx.request({
      url: "http://localhost:8085/admin/class/page/" + that.data.currentPage,
      method: 'GET',
