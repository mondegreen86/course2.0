...
Page({
  ...
  add:function(){
    wx.navigateTo({
      url: '/pages/second_level/courseStudent/add/add',
    })
  ...
  editMessage: function(e) {
    ...
    // 例如：跳转到编辑页面并传递ID参数
    wx.navigateTo({
      url: `/pages/second_level/courseStudent/edit/edit?id=${id}&cname=${cname}&sname=${sname}`
    });
  },
  getInputValue(e) {
    this.setData({
  ...
  loadData: function() {
    ...
    var that = this;
    wx.request({
      url: "http://localhost:8085/admin/student/course/page/" + that.data.currentPage,
      method: 'GET',
      data:{"studentName":this.data.inputValue},
      success: function(res) {
